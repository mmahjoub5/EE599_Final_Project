...
% H = 10;
% RMPC(H, numberOfRandomSamples, xinit, goalX, goalY,  centerX, centerY, innerR, outerR)
while true
    y = RMPC(10, 4000, xinit, goalX, goalY, [0,0], [0,0], 20 , 50)
    if (xinit(1) - params.goalX)^2 < 0.01 && (xinit(2) - params.goalY)^2 < 0.01
        break
    end
   
    if y ~= [-999999]
        result = fourDim_dynamics(xinit(1), xinit(2), xinit(3), xinit(4),y(1),y(2), 0.01, [.2,.2]);
        traj = [traj, result'];
        xinit = result';
        plotTraj(traj, radii, center)
    else
        break
    end
end
...
function y = plotTraj(traj, radii, center)
    figure(1);
    plot(traj(1, :), traj(2, :), 'color', 'b', 'LineWidth', 2);
    hold on;
    viscircles(center, radii, 'Color','r')
    hold off
    xlim([-50, 50]);
    ylim([-50, 50])
